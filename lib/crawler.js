...
class Crawler {
  ...
  async getAllBoards() {
    ...
    // TODO: go into [目录] and crawl boards
    ...
    async function startFromPostId(page, startId) {
      ...
      while (true) {
        ...
        for (let post of posts) {
          ...
          let html = await page.$$eval('html', html => html.length !== 0 ? html[0].innerHTML : '')
          if (html === '') continue
          while (true) {
            let html = await page.$$eval('html', html => html.length !== 0 ? html[0].innerHTML : '')
          ...
          // NOTE: 可能是损坏的xml. shame on bbs!
          if (Object.keys(post.data).every(k => post.data[k].length === 0)) {
            let html = await page.$$eval('body', body => body.length !== 0 ? body.innerHTML : '')
            Logging.error("o%", post)
            Logging.error(html)
            //process.exit(1)
          }
        }
